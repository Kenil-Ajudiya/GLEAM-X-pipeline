...
export GXABSMEMORY=360           # Absolute memory a machine should be considered to have in GB, e.g. 60. This value is submitted to slurm via "--mem=${GXABSMEMORY}"
...
export GXMEMORY=340              # Typical memory a program should use in GB, e.g. 50. This is used for tasks like 'calibrate' and 'wsclean' to attempt to limit
...
export GXNCPUS=38               # Number of CPUs of each machine, e.g. 48. For tasks that have a 'core' like option this value is passed. 
export GXNLCPUS=38              # Number of logical CPUs on each machine, e.g. 48. This option may be phased out. 
export GXNPCPUS=38              # Number of physical CPUs on each machine, e.g. 24. This is meant to count only the physical cores available. 
