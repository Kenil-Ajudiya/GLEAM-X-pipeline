...
# Basic configuration
cluster="setonix"            # System-wide name of cluster, e.g. "garrawarla". This does not have to have a one-to-one correspondance to the
                                # actual system name.
...
export GXBASE="/scratch/projects/mwasci/${GXUSER}/GLEAM-X-pipeline" # Path to base of GLEAM-X Pipeline where the repository was 'git clone' into, e.g. "/astro/mwasci/tgalvin" 
...
export GXABSMEMORY=150           # Absolute memory a machine should be considered to have in GB, e.g. 60. This value is submitted to slurm via "--mem=${GXABSMEMORY}"
...
export GXMEMORY=140              # Typical memory a program should use in GB, e.g. 50. This is used for tasks like 'calibrate' and 'wsclean' to attempt to limit
...
export GXNCPUS=30               # Number of CPUs of each machine, e.g. 48. For tasks that have a 'core' like option this value is passed. 
export GXNLCPUS=30              # Number of logical CPUs on each machine, e.g. 48. This option may be phased out. 
export GXNPCPUS=30              # Number of physical CPUs on each machine, e.g. 24. This is meant to count only the physical cores available. 
export GXNCPULINE="--ntasks-per-node=${GXNCPUS}"            # Informs the SLURM request how many CPUs should be allocated, e.g. "--ntasks-per-node=${GXNPCPUS}" 
                                # If unset the SLURM default will be used. For tasks that are not parallelisable (apply_cal, uvflag), 
                                # this option will be overwritten to ensure a single core is used.
